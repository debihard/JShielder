...
install_nginx_modsecurity(){
  ...
  cd nginx-1.16.0/
  ./configure --user=www-data --group=www-data --with-pcre-jit --with-debug --with-http_ssl_module --add-module=/root/JShielder/UbuntuServer_18.04LTS/src/ModSecurity/ModSecurity-nginx
  make
  make install
...
if [ "$psad_answer" == "y" ]; then
     ...
     echo -e "127.0.0.1/8 0\n255.255.255.255 0" >> /etc/psad/auto_dl
     psad --sig-update
